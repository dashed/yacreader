...
### C3 — Critical (NEW): the Stump GraphQL client targets a non-existent schema — ✅ FIXED
**Status: FIXED** (commit follows this addendum). The client now matches the real schema: `libraryById(id){ media { readProgress{page updatedAt} readHistory{completedAt} } }` for fetch; `updateMediaProgress(id, input:{paged:{page}})` (union return) for page push; `markMediaAsComplete(id, isComplete:true)` for completion. `StumpMedia` carries `read_progress: Option<ActiveReadingSession>` + `read_history: Vec<FinishedReadingSession>`; `is_complete()` = non-empty history; `current_page()` returns `pages` for a completed-no-active-session comic (so it isn't misread as page 0). The confirmed `markMediaAsComplete` non-idempotency hazard (resolver: "we don't clean up existing ones") is guarded by `push_mark_complete = final_complete && !stump_complete` — proven by an e2e test that flips Stump in-progress→complete across two syncs and asserts exactly one completion push. A tripwire unit test fails if any fictional name (`readProgresses`/`putMediaCompletion`/`UpdateMediaProgress`) reappears. 72 tests pass (default + `--features ffi`); `grep` confirms zero fictional names remain. Durable anti-drift recommendation still open: generate types from the schema (`graphql-client`) or add a live smoke test.
...
## Addendum 2 (2026-05-29) — remaining Highs & Mediums fixed

All remaining audit Highs/Mediums are now resolved on `feat/stump-sync-phase2` (84 tests pass, default + `--features ffi`; grep still shows zero fictional schema names). Lead-verified by reading the safety-critical logic line-by-line.

- **H6 — conflict policy reconciled (hybrid, user-approved).** The code keeps the monotonic default (page = max, read = OR — never silently regressed/cleared) and now ALSO detects an **intentional regression**: if exactly one side's page drops below the last-synced page AND that *same side's* source timestamp is newer than the baseline recorded for that side (a per-side, same-clock compare — robust to cross-server skew), the re-read is propagated. `sync_state` now records the converged values + real per-side timestamps (fixes **M2**) and is read back at the start of reconciliation. `write_yac_progress` keeps its C2-safe monotonic default and only lowers a page / clears `read` under an explicit `allow_regression` flag, which is set **only** for a validated Stump-side regression. `ts_newer` returns false on any missing/unparseable timestamp, so stale/unverifiable data can never regress (C2 guard intact — proven by `test_stale_lower_page_does_not_regress`). Stump `read_history` is append-only, so YAC→Stump "unmark as read" is a documented limitation. Doc §5.3/§Phase-3 updated to describe this real policy (no more "LWW").
- **M1 — secret leak closed.** `Config` has a hand-written `Debug` that prints `api_key: "<redacted>"`; the FFI `SyncConfig` no longer derives `Debug`. Test asserts the key value never appears in `{:?}`.
- **M3 — double-fetch removed.** `push_library` fetches the `.ydb` comics and Stump media once and passes them into `build_mappings` (now sync, no internal fetch).
- **M4 — remapping fixed.** `media_mapping` UNIQUE is now `(library_mapping_id, yac_comic_info_id)` with `INSERT … ON CONFLICT DO UPDATE`; `get_stump_id`/`get_yac_id` are deterministic (`ORDER BY matched_at DESC, id DESC LIMIT 1`). A one-time auto-migration drops the legacy derived tables (cache only — never `library_mapping`) and bumps `PRAGMA user_version` to 2.
- **M5 — path matching fixed.** Library-prefix matching now requires a real path-component boundary (`/srv/comics` no longer matches `/srv/comics-extra`) and collapses duplicate slashes. Case/Unicode-NFC differences remain a documented limitation (no new dependency).
- **M6 — moot + cleaned.** C3's real schema scopes `readProgress`/`readHistory` to the API-key user, so no client-side user filter is needed; the dead `user_id()` accessor was removed (the field is retained, documented, for logging/future use).
- **M7 — FFI panic safety.** Every `extern "Rust"` entry point is wrapped in `catch_unwind`; a panic returns an error/safe-default instead of aborting the host server. Test proves a panicking body maps to the error variant.
- **M8 — doc §9.6 reconciled** to the real `corrosion_add_cxxbridge` wiring (non-optional `cxx`, target `stump_sync`, include `"stump_sync_bridge/lib.h"`, `STUMP_SYNC_SKIP_CXX_BUILD`), plus the §9.5 include path.

**Status: all audit findings addressed** except the still-open hardening recommendation (generate GraphQL types from the schema / add a live-Stump smoke test) and the broader test-coverage backfill (init-twice, periodic-timer timing). Both are noted for future work, not regressions.

...
// Generated by corrosion_add_cxxbridge(stump_sync_bridge ... FILES lib.rs). The
// cxxbridge CLI names the header "lib.h" (NAME_WE), and the include prefix is the
// bridge target name.
#include "stump_sync_bridge/lib.h"
#endif
...
int start(QCoreApplication &app, QCommandLineParser &parser, const QStringList &args, QSettings *settings, QTextStream &qout)
{
...
#ifdef ENABLE_STUMP_SYNC
    {
        ...
        quint64 syncInterval = syncSettings.value("syncIntervalSecs", 300).toULongLong();

        // Default the mapping DB under the settings dir if unset, so we never
        // open "" (which would create a throwaway temp DB and lose mappings).
        if (mappingDbPath.isEmpty()) {
            mappingDbPath = YACReader::getSettingsPath() + "/stump_sync_mappings.db";
        }

        if (!stumpUrl.isEmpty() && !apiKey.isEmpty()) {
            auto toRust = [](const QString &s) {
                QByteArray utf8 = s.toUtf8();
                return rust::String(utf8.constData(), utf8.size());
            };

            stump_sync::SyncConfig syncConfig;
            syncConfig.stump_url = toRust(stumpUrl);
            syncConfig.api_key = toRust(apiKey);
            syncConfig.user_id = toRust(userId);
            syncConfig.sync_interval_secs = syncInterval;
            syncConfig.mapping_db_path = toRust(mappingDbPath);

            // One entry per YACReader library. yac_library_id is the numeric
            // (legacy) id used in `/v2/library/<id>/` URLs and carried by the
            // comicUpdated signal. stump_library_* are left empty so the Rust
            // side auto-discovers them, unless an explicit override is set under
            // [StumpSync] override/<id>/stumpLibraryId|stumpLibraryPath.
            auto libs = DBHelper::getLibraries().getLibraries();
            for (const auto &lib : libs) {
                stump_sync::LibraryEntry entry;
                entry.yac_library_id = static_cast<int64_t>(lib.getLegacyId());
                entry.ydb_path = toRust(lib.getPath() + "/.yacreaderlibrary/library.ydb");
                entry.library_root = toRust(lib.getPath());

                const QString overridePrefix = QString("override/%1/").arg(lib.getLegacyId());
                entry.stump_library_id = toRust(syncSettings.value(overridePrefix + "stumpLibraryId").toString());
                entry.stump_library_path = toRust(syncSettings.value(overridePrefix + "stumpLibraryPath").toString());

                syncConfig.libraries.push_back(std::move(entry));
            }
